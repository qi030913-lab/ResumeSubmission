...
class TaskExecutor:
    ...
    async def _execute(self, task_id: str) -> dict:
        with self.session_factory() as session:
            ...
            adapter, driver = adapter_registry.build(task.platform_code, task.platform_type)
            ...
            run = ApplicationRun(task_id=task.id, driver_type="mock_android", adapter_code=adapter.platform_code)
            session.add(run)
            session.add(
                ApplicationLog(
                    task_id=task.id,
            ...
            screenshot_path = await driver.take_screenshot()
            run.finished_at = datetime.now(UTC)
            run.result = "succeeded"
            run.screenshot_urls = [screenshot_path]
            run.raw_output = {"steps": outputs, "driver_operations": driver.operations}
